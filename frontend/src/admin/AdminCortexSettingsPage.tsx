...
import { CortexOverview } from "./cortexAdminTypes";
...
export default function AdminCortexSettingsPage() {
  ...
  if (ingestionQ.isPending) return <p className="text-sm text-stone-600">Loading settings…</p>;
  if (ingestionQ.isError) return <p className="text-sm text-red-700">{(ingestionQ.error as Error).message}</p>;

  const sched = ingestionQ.data.global_scheduler;
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 text-xs text-stone-600">{sched.operator_mode_label}</p>
