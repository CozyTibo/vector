...
export default function Step3CanonicalResetPanel({ tenantId }: { tenantId: string }) {
  ...
  const runStep3Reset = async () => {
    ...
    try {
      ...
      setResetOk(
        `Removed relationships; artifacts ${body.deleted_artifacts}; actors ${body.deleted_actors}; cursors ${body.deleted_step3_canonical_cursors}.`,
      );
      setResetOpen(false);
      setResetPhrase("");
        ...
        >
          <div className="max-w-lg rounded-lg border border-stone-200 bg-white p-5 shadow-lg">
            <h2 id="step3-reset-title" className="text-lg font-semibold text-stone-900">
              Reset all execution graph data for this workspace?
            </h2>
            <p className="mt-2 text-sm text-stone-700">
              ...
              <strong>step3_canonical_cursor</strong> rows for this workspace (all connections). Raw
              ingestion and projections are <strong>not</strong> deleted.
            </p>
            ...
            <div className="mt-4 flex flex-wrap justify-end gap-2">
              ...
              >
                {resetPending ? "Resetting…" : "Reset graph data"}
              </button>
            </div>
          </div>
        </div>
