...
const INSPECT_LENS_PATH: Record<string, string> = {
  ingestion: "ingestion",
  runtime: "runtime",
  identity: "inspect/identity",
  graph: "inspect/graph",
  retrieval: "inspect/retrieval",
  queues: "queues",
};
  ...
  return (
    <section className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
      ...
      <ul className="mt-3 space-y-2">
        {facts.map((fact) => {
          const segment = fact.inspect_lens ? INSPECT_LENS_PATH[fact.inspect_lens] ?? "overview" : null;
          return (
