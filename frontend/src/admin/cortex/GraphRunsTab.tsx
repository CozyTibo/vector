...
import type { GraphPassRunItem, GraphPassRuns } from "../cortexAdminTypes";
...
function triggerLabel(sourceTrigger: string): string {
  ...
  if (key === "ingestion_complete") return "Ingestion";
  if (key === "manual_rebuild") return "Rebuild";
...
export function GraphRunsTab() {
  ...
  const runsQ = useQuery({
    queryKey: ["admin-cortex-graph-runs", tenantId, page],
    queryFn: () => {
      const params = new URLSearchParams({
        limit: String(PAGE_SIZE),
        offset: String(offset),
      });
      return adminJson<GraphPassRuns>(`/admin/tenants/${tenantId}/cortex/graph/runs?${params}`);
    },
  ...
  return (
    <section className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
      <div>
        <h2 className="text-base font-semibold text-stone-900">Graph projection passes</h2>
        <p className="text-sm text-stone-600">Extract and enrich runs · newest first</p>
      </div>
      ...
      {runsQ.isPending && !data ? (
        ...
        <>
          <p className="mt-3 text-xs text-stone-500">
            {total === 0
              ? "No graph passes recorded for this workspace yet."
              : `Showing ${from.toLocaleString()}–${to.toLocaleString()} of ${total.toLocaleString()} pass(es)`}
          </p>
          <GraphRunsTable runs={data?.items ?? []} />
          {total > PAGE_SIZE ? (
...
function GraphRunsTable({ runs }: { runs: GraphPassRunItem[] }) {
  if (runs.length === 0) {
    return <p className="mt-4 text-sm text-stone-500">No graph passes recorded for this workspace yet.</p>;
  }
      ...
      <table className="min-w-full text-xs">
        <thead className="bg-stone-50 text-left text-stone-700">
          <tr>
            ...
            <th className="px-2 py-2">Upserted</th>
            <th className="px-2 py-2">Enriched</th>
            <th className="px-2 py-2">Unresolved</th>
        ...
        <tbody>
          {runs.map((run) => (
            <tr key={run.id} className="border-t border-stone-100">
              ...
              <td className="px-2 py-2 tabular-nums">{fmtStat(run.stats, "processed")}</td>
              <td className="px-2 py-2 tabular-nums">{fmtStat(run.stats, "edges_upserted")}</td>
              <td className="px-2 py-2 tabular-nums">{fmtStat(run.stats, "edges_enriched")}</td>
              <td className="px-2 py-2 tabular-nums">{fmtStat(run.stats, "unresolved_refs")}</td>
