...
type SlackPeopleMultiConfirmPanelProps = {
  ...
  /** When false, primary CTA stays enabled with an empty list (team step). */
  requireNonEmpty: boolean;
};

export default function SlackPeopleMultiConfirmPanel({
  priorChatMessages,
  ...
  userDisplayName,
  requireNonEmpty,
}: SlackPeopleMultiConfirmPanelProps) {
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="shrink-0 border-t border-zinc-100/90 bg-white/90 px-4 pb-6 pt-3 backdrop-blur-sm sm:px-5">
        ...
        <div className="mb-4 rounded-2xl border border-zinc-200/90 bg-zinc-50/80 px-4 py-3">
          ...
          {requireNonEmpty && members.length === 0 ? (
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-center">
          ...
          <button
            type="button"
            disabled={submitting || (requireNonEmpty && members.length === 0)}
            onClick={onContinue}
            className={
