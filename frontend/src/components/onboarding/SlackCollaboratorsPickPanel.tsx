...
type SlackCollaboratorsPickPanelProps = {
  ...
  /** Bumps when server state should re-seed local picks (e.g. return from confirm step). */
...
export default function SlackCollaboratorsPickPanel({
  priorChatMessages,
  members,
  membersLoading,
  membersError,
  ...
  onContinue,
  userDisplayName,
}: SlackCollaboratorsPickPanelProps) {
  const ids = useId();
  const inputId = `${ids}-collab-input`;
  const listboxId = `${ids}-collab-list`;
  ...
  const listMessages = useMemo(() => {
    const t = Date.now();
    return [...priorChatMessages, ...slackCollaboratorsPickIntroMessages(t)];
  }, [priorChatMessages]);
      ...
      <div className="shrink-0 border-t border-zinc-100/90 bg-white/90 px-4 pb-6 pt-3 backdrop-blur-sm sm:px-5">
        ...
        <div className="space-y-3">
          <p className="text-center text-[12px] leading-relaxed text-zinc-500">
            ...
            <span className="font-medium text-zinc-700">@username</span>, tap a row to add. Remove people with{" "}
            <span className="font-medium text-zinc-700">×</span> on their chip.
          </p>
          {picked.length > 0 ? (
            ...
            <p className="text-center text-[12px] text-amber-800">Pick at least one person to continue.</p>
          )}
