...
def _classify_drain_outcome(
    *,
    ...
    if topology_wait and total_succeeded == 0:
...
def drain_forward_progress_backlog(
    ...
    *,
    ...
    """Drain canonical backlog with spin detection, deferrals, and success-based outcomes."""
    ...
    spin_limit = max(1, int(cfg.cortex_canonical_forward_progress_zero_progress_batch_limit))
    cooldown_s = max(5, int(cfg.cortex_canonical_topology_wait_cooldown_seconds))
    ...
    empty_pass_rotations = 0
    pass_count = len(all_canonical_passes()) or 1
    scoped_drain = bool(
    ...
    while batches_run < max_batches:
        raw_batch = materialize_stub_backlog(
            ...
            topology_cooldown_seconds=cooldown_s,
        )
        last_batch_meta = raw_batch.get("forward_progress") if isinstance(raw_batch.get("forward_progress"), dict) else {}
        ...
        if selected_n == 0 and attempted_n == 0:
            if scoped_drain:
                ...
                topology_wait = True
                break
            empty_pass_rotations += 1
            batches_run -= 1
            if empty_pass_rotations >= pass_count:
                break
            continue

        empty_pass_rotations = 0

        # Zero-progress spin: selected rows but only topology skip/defer, no failures.
        if succeeded_n == 0 and len(failures_batch) == 0 and (attempted_n > 0 or topology_skipped_n > 0):
            zero_progress_streak += 1
            if zero_progress_streak >= spin_limit:
                topology_wait = True
                break
        else:
            zero_progress_streak = 0
    ...
    return {
        ...
        "topology_wait": topology_wait,
        "zero_progress_spin_detected": topology_wait and total_succeeded == 0,
