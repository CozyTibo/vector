...
def _github() -> dict[str, Any]:
    ...
    return _connector_bundle(
        "github",
        maturity_level=3,
        historical_backfill_summary="Repo-scoped PR/review/comment/commit/check/workflow/deployment/branch/tag streams progress with checkpointed page continuity.",
...
def _calls() -> dict[str, Any]:
    resources = [
        ...
        _row(
            ...
            canonicalization="none",
            status="in_progress",
            notes="Transcript blobs persisted when provider payload includes transcript object.",
        ),
        _row(
            ...
            canonicalization="none",
            status="in_progress",
            notes="Transcript segment rows extracted from transcript payload when segment arrays exist.",
        ),
        _row(
            "calls.recording",
            coverage="partial",
