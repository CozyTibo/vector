"""Postgres-authoritative convergence lease operations."""
...
def get_convergence_lease_v1(
    session: Session,
    *,
    tenant_id: uuid.UUID,
) -> CortexTenantConvergenceLease | None:
...
def _get_or_create_lease(
    ...
    *,
    ...
    row = get_convergence_lease_v1(session, tenant_id=tenant_id)
    if row is not None:
        return row
...
def mark_tenant_dirty_v1(
    ...
    *,
    ...
    """Bump obligation epoch and mark tenant as needing convergence."""
    now = _now()
    ...
    session.flush()
    _LOGGER.info(
        "convergence_lease_marked_dirty tenant_id=%s obligation_epoch=%s reason=%s",
        tenant_id,
...
def try_acquire_convergence_lease_v1(
    ...
    *,
    ...
    if row.status == LEASE_STATUS_RUNNING:
        ...
        _LOGGER.warning(
            "convergence_lease_expired_recovered tenant_id=%s",
            tenant_id,
        )
...
def complete_convergence_lease_v1(
    ...
    *,
    ...
    """Release lease after a convergence slice; stay dirty if obligation advanced."""
    now = _now()
    ...
    if int(lease.obligation_epoch) > int(lease.target_epoch):
        ...
        _LOGGER.info(
            "convergence_lease_still_dirty tenant_id=%s obligation_epoch=%s target_epoch=%s",
            lease.tenant_id,
        ...
        _LOGGER.info(
            "convergence_lease_idle tenant_id=%s target_epoch=%s",
            lease.tenant_id,
            lease.target_epoch,
        )
    lease.updated_at = now
...
def list_tenants_for_convergence_sweep_v1(
    ...
    *,
    ...
    """Tenants that owe convergence work now (authoritative scheduler input)."""
    cfg = settings or get_settings()
    now = _now()
...
def resume_convergence_from_waiting_v1(
    ...
    *,
    ...
    row.updated_at = _now()
    session.flush()
    return {"tenant_id": str(tenant_id), "status": row.status, "phase_cursor": phase_cursor}
