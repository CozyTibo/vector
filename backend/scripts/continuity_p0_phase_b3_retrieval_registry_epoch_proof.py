#!/usr/bin/env python3
"""Phase B step B3 — island registry last_retrieval_epoch publish wiring prod proof."""
...
from vector.domains.cortex.substrate_pipeline.continuity_p0_retrieval_registry_epoch import (
    ...
    snapshot_retrieval_registry_epoch_v1,
...
def main() -> int:
    ...
    parser.add_argument(
        "--drive-sync",
        action="store_true",
        help="Rebuild island registry from published epoch (fixes stale last_retrieval_epoch)",
    )
    ...
    registry_drive: dict | None = None

    with SessionLocal() as session:
        snapshot = snapshot_retrieval_registry_epoch_v1(session, tenant_id=tenant_id)
        stale = int(
            (snapshot.get("registry_epoch_audit") or {}).get("registry_rows_stale_vs_published")
            or 0
        )
        need_drive = args.drive_sync or stale > 0
        if need_drive and not trace_only:
            registry_drive = record_retrieval_publish_on_island_registry_v1(
                session,
                tenant_id=tenant_id,
            )
            if not args.dry_run and registry_drive.get("synced"):
                session.commit()
            snapshot = snapshot_retrieval_registry_epoch_v1(session, tenant_id=tenant_id)
    ...
    audit = dict(snapshot.get("registry_epoch_audit") or {})
    step_record = {
        "validated_at": datetime.now(UTC).isoformat(),
        "closure_git_sha": closure_sha,
        "tenant_id": str(tenant_id),
        "p0_b3_pass": proof["p0_b3_pass"],
        "checks": proof["checks"],
        ...
        "registry_rows_stale_vs_published": audit.get("registry_rows_stale_vs_published"),
        "primary_island_epoch_aligned": audit.get("primary_island_epoch_aligned"),
    ...
    saved = save_p0_step_baseline_v1(
        baseline_path,
        baseline,
        step_key="step_b3_retrieval_registry_epoch",
    ...
    return 0 if proof["p0_b3_pass"] else 1
