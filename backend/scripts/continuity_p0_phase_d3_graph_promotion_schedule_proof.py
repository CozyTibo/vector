#!/usr/bin/env python3
"""Phase D step D3 — graph promotion on convergence worker schedule prod proof."""
...
from vector.domains.cortex.substrate_pipeline.continuity_p0_phase_d3_graph_promotion_schedule import (
    ...
    snapshot_d3_graph_promotion_truth_v1,
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Phase D.3 graph promotion convergence schedule prod proof")
    ...
    parser.add_argument("--no-drive", action="store_true", help="Skip live promotion drain on prod tenant")
    ...
    with SessionLocal() as session:
        snapshot = snapshot_d3_graph_promotion_truth_v1(
            session,
            tenant_id=tenant_id,
            drive_promotion=not args.no_drive,
        )
    ...
    if args.dry_run:
        return 0 if proof["p0_d3_pass"] else 1
    ...
    step_record = {
        ...
        "p0_d3_pass": proof["p0_d3_pass"],
        "checks": proof["checks"],
        "checks_advisory": proof.get("checks_advisory"),
        "wiring_ok": wiring.get("wiring_ok"),
        "auth_links_created_recent_48h": trend.get("auth_links_created_recent_48h"),
        "graph_promoted_edge_count": snapshot.get("graph_promoted_edge_count"),
        "use_deployed_closure": args.use_deployed_closure,
    }
    saved = save_p0_step_baseline_v1(
        baseline_path,
        baseline,
        step_key="step_d3_graph_promotion_on_convergence_schedule",
