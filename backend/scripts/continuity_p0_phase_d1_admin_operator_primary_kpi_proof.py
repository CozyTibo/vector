#!/usr/bin/env python3
"""Phase D step D1 — admin drainable + island KPI prod proof."""
...
from vector.domains.cortex.substrate_pipeline.continuity_p0_phase_d1_admin_operator_primary_kpi import (
    ...
    snapshot_d1_operator_primary_kpi_v1,
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Phase D.1 admin operator primary KPI prod proof")
    ...
    with SessionLocal() as session:
        snapshot = snapshot_d1_operator_primary_kpi_v1(session, tenant_id=tenant_id)
    ...
    if args.dry_run:
        return 0 if proof["p0_d1_pass"] else 1
    ...
    step_record = {
        ...
        "p0_d1_pass": proof["p0_d1_pass"],
        "checks": proof["checks"],
        "checks_advisory": proof.get("checks_advisory"),
        "wiring_ok": wiring.get("wiring_ok"),
        "primary_metric_key": kpi.get("primary_metric_key"),
    ...
    saved = save_p0_step_baseline_v1(
        baseline_path,
        baseline,
        step_key="step_d1_admin_operator_primary_kpi",
