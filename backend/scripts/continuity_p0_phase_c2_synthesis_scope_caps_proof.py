#!/usr/bin/env python3
"""Phase C step C2 — per-island scope caps + orchestrator fail-loud prod proof."""
...
from vector.domains.cortex.substrate_pipeline.continuity_p0_phase_c2_synthesis_scope_caps import (
    ...
    verify_c2_synthesis_scope_caps_wiring_v1,
)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Phase C.2 per-island scope caps + orchestrator fail-loud prod proof"
    )
    ...
    parser.add_argument(
        "--drive-phase08",
        action="store_true",
        help="Re-run phase 08 on latest pipeline with completed phase 07 (applies C2 caps)",
    )
    ...
    with SessionLocal() as session:
        snapshot = snapshot_c2_synthesis_scope_caps_v1(session, tenant_id=tenant_id)
        ...
        need_drive = (
            args.drive_phase08
            or primary_artifacts < 2
            or cap_audit_slices < 1
        )
        if need_drive and not trace_only and not args.dry_run:
            phase08_drive = drive_phase08_truth_on_latest_run_v1(session, tenant_id=tenant_id)
            session.commit()
            snapshot = snapshot_c2_synthesis_scope_caps_v1(session, tenant_id=tenant_id)
    ...
    return 0 if proof["p0_c2_pass"] else 1
