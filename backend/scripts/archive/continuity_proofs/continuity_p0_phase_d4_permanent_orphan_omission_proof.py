#!/usr/bin/env python3
"""Phase D step D4 — permanent orphan deferrals as bounded omission prod proof."""
...
from vector.domains.cortex.substrate_pipeline.continuity_p0_phase_d4_permanent_orphan_omission import (
    ...
    snapshot_d4_permanent_orphan_omission_truth_v1,
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Phase D.4 permanent orphan omission doctrine prod proof"
    )
    ...
    with SessionLocal() as session:
        snapshot = snapshot_d4_permanent_orphan_omission_truth_v1(session, tenant_id=tenant_id)
    ...
    if args.dry_run:
        return 0 if proof["p0_d4_pass"] else 1
    ...
    omission = dict((snapshot.get("snapshot") or {}).get("deferral_omission") or {})
    step_record = {
        "validated_at": datetime.now(UTC).isoformat(),
        "closure_git_sha": closure_sha,
        "tenant_id": str(tenant_id),
        "p0_d4_pass": proof["p0_d4_pass"],
        "checks": proof["checks"],
        "checks_advisory": proof.get("checks_advisory"),
        "wiring_ok": wiring.get("wiring_ok"),
        "permanent_orphan_count": omission.get("permanent_orphan_count"),
        "deferral_total": omission.get("deferral_total"),
        "use_deployed_closure": args.use_deployed_closure,
    }
    saved = save_p0_step_baseline_v1(
        baseline_path,
        baseline,
        step_key="step_d4_permanent_orphan_omission_doctrine",
