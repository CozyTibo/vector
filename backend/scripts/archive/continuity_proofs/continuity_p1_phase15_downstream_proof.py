#!/usr/bin/env python3
"""Phase 1 step 1.5 — drive 06→TCRE→07→08 on prod and record phase receipts."""
...
os.environ.setdefault("CORTEX_SYNTHESIS_PIPELINE_MAX_SCOPES", "8")
...
from vector.domains.cortex.substrate_pipeline.continuity_p1_downstream import (
    ...
    complete_queued_tcre_jobs_for_pipeline_v1,
    evaluate_p1_5_downstream_proof_v1,
    prepare_pipeline_for_downstream_v1,
    run_downstream_execution_slices_v1,
    snapshot_downstream_phase_receipts_v1,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase 1.5 downstream chain prod proof")
    ...
    parser.add_argument("--max-tcre-jobs", type=int, default=8)
    parser.add_argument("--proof-only", action="store_true", help="Evaluate only; no chain drive")
    ...
    chain_drive: dict = {"skipped": True}

    if not args.proof_only:
        with SessionLocal() as session:
            prepare = prepare_pipeline_for_downstream_v1(
                session,
                tenant_id=tenant_id,
                pipeline_run_id=pipeline_run_id,
            )
            tcre_drain = complete_queued_tcre_jobs_for_pipeline_v1(
                session,
                tenant_id=tenant_id,
                pipeline_run_id=pipeline_run_id,
                limit=args.max_tcre_jobs,
            )
            slices = run_downstream_execution_slices_v1(
                session,
                tenant_id=tenant_id,
            ...
            chain_drive = {
                "prepare": prepare,
                "tcre_drain": tcre_drain,
                "execution_slices": slices,
            }

    with SessionLocal() as session:
        receipts = snapshot_downstream_phase_receipts_v1(
            session,
            tenant_id=tenant_id,
            pipeline_run_id=pipeline_run_id,
        )
        tcre_footprint = snapshot_tcre_execution_footprint_v1(
            session,
            tenant_id=tenant_id,
            pipeline_run_id=pipeline_run_id,
        )
    ...
    baseline["step_1_5_p1de_downstream"] = proof
    baseline["p1_5_closure_git_sha"] = closure_sha
    save_continuity_p0_baseline_v1(baseline_path, baseline)

    print(json.dumps(proof, indent=2, default=str))
    print(f"wrote {baseline_path}", file=sys.stderr)

    if not proof.get("p1_5_pass"):
        print("FAIL: step 1.5 not satisfied", file=sys.stderr)
        return 1
    return 0
