...
def test_admin_clear_derived_wipes_canon_keeps_raw_and_enqueues_pass(
    monkeypatch: pytest.MonkeyPatch,
    client: TestClient,
    db_session: Session,
) -> None:
    ...
    monkeypatch.setattr(
        cortex_runtime.poll_cortex_passes_task,
        "delay",
        lambda: poll_called.append(True),
    )
    try:
        tid, uid = _tenant_with_owner(db_session)
        _seed_raw_and_canon(db_session, tid, uid)
        ...
        assert body["raw_ingestion_rows_remaining"] == raw_before
        assert body["deleted_rows_total"] >= 1
