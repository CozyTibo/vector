...
Create a [Slack app](https://api.slack.com/apps) and enable **OAuth & Permissions**. Set **Redirect URL** to **`SLACK_CALLBACK_URL`** in full (e.g. `https://<ngrok-host>/slack/callback` for local HTTPS). Bot scopes must match **`SLACK_BOT_SCOPES`**. Put **`SLACK_CLIENT_ID`**, **`SLACK_CLIENT_SECRET`**, and optionally **`SLACK_SIGNING_SECRET`** in `.env`, run **`make migrate`**, then use **Connect Slack** in the UI or `GET /connectors/slack/install`. The OAuth return is **`GET /slack/callback`** (mounted at app root so the URL matches your env). Redirect to the app includes `slack_connected=1` or `slack_error=…`.
