...
- ~~Phase 01 Step 2 polling/orchestration~~ **Done** — Celery Beat `vector.cortex.ingestion.scheduler_tick`, `cortex_live` queue for `vector.cortex.ingestion.run_sync`, `vector.domains.cortex.ingestion.scheduler` + settings `CORTEX_INGESTION_SCHEDULER_*` / `CORTEX_INGESTION_MIN_GAP_SECONDS` (see `DOCS/cortex/MASTER_TRACKER.md`). **Follow-on:** Step 3 replay enforcement, Step 4+ contract validation.
